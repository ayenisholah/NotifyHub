...
- Did: normalized `main` into 42 linear Conventional Commits with identical final tree, author identity, and timestamps; flattened the two historical topic merges; enabled squash-only merges, automatic branch deletion, neutral Git convention enforcement, and a no-bypass `main` ruleset requiring pull requests plus conventions, verification, containers, and reliability; removed the production reviewer gate while retaining its `main`-only policy and canonical URL; added serialized automatic deployment of exact full-SHA images, protected custom-format PostgreSQL/configuration backups, checksum and scratch-restore validation, automatic image/Compose rollback without automatic database restoration, runner-owned locked backup/retention cron, seven-day PostgreSQL/BullMQ/Mailpit retention, and the production recovery runbook. PR #4 delivered the implementation and PR #5 corrected the privileged-SSH/runner-ownership boundary exposed by the first guarded attempt.
- Verify: PASS ([normalization run `29358377431`](https://github.com/ayenisholah/NotifyHub/actions/runs/29358377431) preserved tree `259b8ba09bdad5d9ab63a830cbe84d4d9dc4d7e6` across 42 commits with zero merges; [production run `29369343184`](https://github.com/ayenisholah/NotifyHub/actions/runs/29369343184) passed conventions, formatting, lint, strict typecheck, builds, smokes, complete unit and PostgreSQL integration suites, retention boundaries and queue-state restoration, the 500-delivery worker-kill gate, Compose/Nginx/image validation, protected backup checksums and scratch restoration, Chromium topology journey, immutable VPS deployment of `20d6ff16d9aedcf6458d191ce66b2d9c7f7894ca`, loopback health, live canonical/social/manifest/robots/sitemap checks, WebSocket inbox updates, dashboard timelines, and tunneled Mailpit delivery. The successful deployment required `.env` mode `600`, installed the `runner` cron block, restored persistent paths to `runner` ownership, and skipped rollback after live acceptance passed.)
- Next: W2D5-3 — Evidence-backed release documentation (M4).
- Blockers/notes: the first production deployment was a bootstrap release and therefore reported no previous release to back up; every later deployment fails closed unless it creates and validates a pre-rollout backup. The earlier run `29368667098` stopped at the deployment-user guard before any rollout or database change and led to the explicit privileged-orchestrator/runner-owner separation. Database restoration remains manual to avoid discarding notifications accepted after a backup.

## 2026-07-15 — W2D5-3 (M4)

- Did: published an evidence-led README with the real live delivery journey, editable fan-out architecture, reliability model, controlled measurement limitations, quickstart, curl and widget integration examples, production operations, and a complete root `package.json` script/workspace reference; added real production-derived cover, GIF, MP4, and architecture assets; finalized the `0.1.0` changelog and release notes; and published GitHub Release `v0.1.0` at the exact accepted production revision. PR #7 delivered the release documentation. External Upwork publication and its repository handoff were explicitly skipped by the owner and are not claimed as completed.
- Verify: PASS ([PR #7](https://github.com/ayenisholah/NotifyHub/pull/7) passed conventions, formatting, lint, strict typecheck, builds, complete unit and PostgreSQL integration suites, immutable image/Compose/Nginx validation, Chromium delivery, protected backup scratch restoration, and worker-kill reliability. [Production run `29373279172`](https://github.com/ayenisholah/NotifyHub/actions/runs/29373279172) created and validated the required pre-rollout backup, deployed exact revision `01fcfe517f5730638c2f976751286997da3891f7`, and passed live canonical/social/manifest/robots/sitemap, API, WebSocket inbox, dashboard timeline, and tunneled Mailpit acceptance without rollback. [GitHub Release `v0.1.0`](https://github.com/ayenisholah/NotifyHub/releases/tag/v0.1.0) is public at that revision with the demo video, cover, and architecture assets.)
- Next: none — M4 closes the implementation plan of record; explicitly deferred ideas remain in the engineering document.
- Blockers/notes: no product blocker remains. The owner waived authenticated Upwork publication on 2026-07-15, so no public Upwork item or URL is asserted. Controlled throughput remains synthetic evidence rather than a production SLO.
